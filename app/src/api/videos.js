...
export const getVideos = async ({ program }, filters = []) => {
  let videos = await program.value.account.video.all(filters.filter((f) => f));

  videos = videos.map((v) => new Video(v.publicKey, v.account));

  await Promise.all(videos.map(async (v) => await v.getMetadata()));
